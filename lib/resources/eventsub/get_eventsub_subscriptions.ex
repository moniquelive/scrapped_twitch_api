defmodule TwitchApi.EventSub.GetEventSubSubscriptions do
  ...
  @spec call(status | type | after_query_param) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  def call(%{status: status}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/eventsub/subscriptions?status=#{status}",
    Headers.config_headers(), nil)
  end

  def call(%{type: type}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/eventsub/subscriptions?type=#{type}",
    Headers.config_headers(), nil)
  end

  def call(%{after: after_query_param}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/eventsub/subscriptions?after=#{after_query_param}",
    Headers.config_headers(), nil)
  end
