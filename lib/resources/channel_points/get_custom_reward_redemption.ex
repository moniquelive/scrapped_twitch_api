defmodule TwitchApi.ChannelPoints.GetCustomRewardRedemption do
  ...
  @spec call(broadcaster_id | reward_id | id | status | sort | after_query_param | first, user_info) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  ...
  def call(%{id: id}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/channel_points/custom_rewards/redemptions?id=#{id}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{status: status}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/channel_points/custom_rewards/redemptions?status=#{status}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{sort: sort}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/channel_points/custom_rewards/redemptions?sort=#{sort}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{after: after_query_param}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/channel_points/custom_rewards/redemptions?after=#{after_query_param}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{first: first}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/channel_points/custom_rewards/redemptions?first=#{first}",
    Headers.config_oauth_headers(user_info), nil)
  end
