defmodule TwitchApi.Analytics.GetGameAnalytics do
  ...
  @spec call(after_query_param | ended_at | first | game_id | started_at | type, user_info) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  def call(%{after: after_query_param}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?after=#{after_query_param}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{ended_at: ended_at}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?ended_at=#{ended_at}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{first: first}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?first=#{first}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{game_id: game_id}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?game_id=#{game_id}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{started_at: started_at}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?started_at=#{started_at}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{type: type}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/games?type=#{type}",
    Headers.config_oauth_headers(user_info), nil)
  end
