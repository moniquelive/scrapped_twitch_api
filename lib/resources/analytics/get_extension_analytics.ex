defmodule TwitchApi.Analytics.GetExtensionAnalytics do
  ...
  @spec call(after_query_param | ended_at | extension_id | first | started_at | type, user_info) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  def call(%{after: after_query_param}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?after=#{after_query_param}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{ended_at: ended_at}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?ended_at=#{ended_at}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{extension_id: extension_id}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?extension_id=#{extension_id}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{first: first}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?first=#{first}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{started_at: started_at}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?started_at=#{started_at}",
    Headers.config_oauth_headers(user_info), nil)
  end

  def call(%{type: type}, user_info) do
    MyFinch.request("GET","https://api.twitch.tv/helix/analytics/extensions?type=#{type}",
    Headers.config_oauth_headers(user_info), nil)
  end
