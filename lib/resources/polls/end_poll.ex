defmodule TwitchApi.Polls.EndPoll do
  ...
  @spec call(%{required(:broadcaster_id) => String.t(), # The broadcaster running polls. Provided broadcaster_id must match the user_id in the user OAuth token.Maximum =>  1
    required(:id) => String.t(), # ID of the poll.
    required(:status) => String.t(), # The poll status to be set. Valid values => TERMINATED =>  End the poll manually but allow it to be viewed publicly.ARCHIVED =>  End the poll manually and do not allow it to be viewed publicly.
    }| nil) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  def call(body_params) do
    MyFinch.request("PATCH","https://api.twitch.tv/helix/polls",
