defmodule TwitchApi.Schedule.GetChannelStreamSchedule do
  ...
  @spec call(broadcaster_id | id | start_time | utc_offset | first | after_query_param) :: {:ok, Finch.Response.t} | {:error, Exception.t}
  ...
  def call(%{id: id}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/schedule?id=#{id}",
    Headers.config_headers(), nil)
  end

  def call(%{start_time: start_time}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/schedule?start_time=#{start_time}",
    Headers.config_headers(), nil)
  end

  def call(%{utc_offset: utc_offset}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/schedule?utc_offset=#{utc_offset}",
    Headers.config_headers(), nil)
  end

  def call(%{first: first}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/schedule?first=#{first}",
    Headers.config_headers(), nil)
  end

  def call(%{after: after_query_param}) do
    MyFinch.request("GET","https://api.twitch.tv/helix/schedule?after=#{after_query_param}",
    Headers.config_headers(), nil)
  end
